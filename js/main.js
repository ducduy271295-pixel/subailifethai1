...
async function seedDefaultsIfNeeded() {
  if (!state.categories.length) {
    state.categories = [
      { id: 'cat-1', name: 'ยาหม่อง & น้ำมันนวด', icon: 'fa-solid fa-prescription-bottle-medical', color: 'emerald', updatedAt: new Date().toISOString() },
      { id: 'cat-2', name: 'สมุนไพรแห้ง & ชาชง', icon: 'fa-solid fa-leaf', color: 'amber', updatedAt: new Date().toISOString() },
      { id: 'cat-3', name: 'อาหารเสริมธรรมชาติ', icon: 'fa-solid fa-capsules', color: 'sky', updatedAt: new Date().toISOString() }
    ];
    for (const cat of state.categories) await saveDBData('categories', cat);
  }
