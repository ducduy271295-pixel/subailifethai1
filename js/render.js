...
export function applyGlobalSettings() {
  ...
  document.getElementById('footer-about-text').innerText =
    settings.about || 'เว็บไซต์รีวิวและรวบรวมสาระสุขภาพสมุนไพรธรรมชาติ';

  document.getElementById('footer-email-text').innerHTML =
    `<i class="fa-solid fa-envelope mr-1"></i> ${escapeHTML(settings.email || 'ducduy271295@gmail.com')}`;
  ...
  document.getElementById('footer-phone-link').href = settings.phone
    ? `tel:${encodeURIComponent(settings.phone)}`
    : '#';
}
...
export function renderCategories() {
  ...
  state.categories.forEach((cat) => {
    ...
    container.insertAdjacentHTML(
      'beforeend',
      `<button onclick="selectCategory('${escapeHTML(cat.id)}')" class="flex-shrink-0 flex items-center space-x-2 px-4 py-2 rounded-2xl text-xs font-bold transition-all border ${
        active
          ? 'bg-slate-900 text-white border-slate-900'
          : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
      }">
        <span class="${active ? 'text-emerald-400' : textColor}">
...
export function filterPosts() {
  ...
  let filtered = state.posts.filter((post) => {
    ...
    const inCategory =
      ...
      post.categoryId === state.selectedCategoryFilter;

    const matches = `${post.title || ''} ${post.content || ''} ${post.productName || ''}`
      .toLowerCase()
      .includes(query);
  ...
  if (sortVal === 'price-high') {
    filtered.sort((a, b) => Number(b.productPrice || 0) - Number(a.productPrice || 0));
  }

  if (sortVal === 'price-low') {
    filtered.sort((a, b) => Number(a.productPrice || 0) - Number(b.productPrice || 0));
  }

  const countLabel = document.getElementById('posts-count-label');
  if (countLabel) {
    countLabel.innerText = `กำลังแสดงทั้งหมด ${filtered.length} รายการ`;
  }
...
export function renderPostsGrid(items) {
  ...
  items.forEach((post) => {
    ...
    grid.insertAdjacentHTML(
      'beforeend',
      `<article class="group bg-white rounded-3xl overflow-hidden border border-slate-200 hover:border-emerald-500 transition duration-300 flex flex-col hover:shadow-lg">
        <div class="relative h-48 sm:h-52 overflow-hidden bg-slate-100 cursor-pointer" onclick="openPostDetail('${escapeHTML(post.id)}')">
          <img src="${img}" alt="${escapeHTML(post.title)}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" onerror="this.src='https://placehold.co/800x600/1e293b/ffffff?text=Image'">
          <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
          <span class="absolute top-4 left-4 inline-flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] font-bold bg-white text-slate-800 shadow-sm">
            <i class="${escapeHTML(cat.icon)} ${catColor}"></i>
            <span>${escapeHTML(cat.name)}</span>
          </span>
        </div>
        ...
        <div class="p-5 flex-grow flex flex-col justify-between space-y-4">
          <div class="space-y-2">
            <span class="text-[10px] text-slate-400 font-bold block">
              <i class="fa-regular fa-clock"></i> ${formatDate(post.createdAt)}
            </span>
            <h3 class="font-bold text-xs sm:text-sm text-slate-950 group-hover:text-emerald-600 cursor-pointer line-clamp-2 transition leading-snug" onclick="openPostDetail('${escapeHTML(post.id)}')">
              ${escapeHTML(post.title)}
            </h3>
            <p class="text-xs text-slate-500 line-clamp-2 leading-relaxed">${escapeHTML(post.content || '')}</p>
          </div>

          <div class="border-t border-slate-100 pt-4 flex justify-between items-center mt-auto">
            <div>
              <span class="text-[10px] text-slate-400 block font-semibold">ราคาประมาณ</span>
              <span class="text-xs sm:text-sm font-extrabold text-amber-600">${Number(post.productPrice || 0).toLocaleString('th-TH')} ฿</span>
            </div>
            <button onclick="openPostDetail('${escapeHTML(post.id)}')" class="px-3 py-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-50 rounded-lg group-hover:bg-emerald-600 group-hover:text-white transition">
              ดูรายละเอียด
            </button>
          </div>
        </div>
...
export function renderBlogsGrid(items) {
  ...
  if (!items.length) {
    grid.insertAdjacentHTML(
      'beforeend',
      `<div class="col-span-full flex flex-col items-center justify-center py-16 bg-white rounded-3xl border border-dashed border-slate-200 text-center">
        <div class="text-slate-300 text-5xl mb-4">
          <i class="fa-solid fa-file-circle-question"></i>
        </div>
        <h3 class="text-sm font-bold text-slate-700">ไม่พบบทความในหมวดหมู่นี้</h3>
        <p class="text-xs text-slate-400 mt-1">ลองเลือกหมวดหมู่อื่นหรือรีเซ็ตการค้นหา</p>
      </div>`
    );
    return;
  }
  ...
  items.forEach((post) => {
    ...
    grid.insertAdjacentHTML(
      'beforeend',
      `<article class="group bg-white rounded-3xl overflow-hidden border border-slate-200 hover:border-teal-500 transition duration-300 flex flex-col hover:shadow-lg">
        <div class="relative h-48 sm:h-52 overflow-hidden bg-slate-100 cursor-pointer" onclick="openPostDetail('${escapeHTML(post.id)}')">
          <img src="${img}" alt="${escapeHTML(post.title)}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" onerror="this.src='https://placehold.co/800x600/1e293b/ffffff?text=Image'">
          <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
          <span class="absolute top-4 left-4 inline-flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] font-bold bg-white text-slate-800 shadow-sm">
        ...
        <div class="p-5 flex-grow flex flex-col justify-between space-y-4">
          <div>
            ...
            <h3 class="font-bold text-xs sm:text-sm text-slate-950 group-hover:text-teal-600 cursor-pointer line-clamp-2 transition leading-snug" onclick="openPostDetail('${escapeHTML(post.id)}')">
              ${escapeHTML(post.title)}
            </h3>
            <p class="text-xs text-slate-500 line-clamp-3 leading-relaxed mt-2">${escapeHTML(post.content || '')}</p>
          </div>

          <div class="border-t border-slate-100 pt-3 text-right">
            <button onclick="openPostDetail('${escapeHTML(post.id)}')" class="text-xs font-bold text-teal-600 hover:underline">
              อ่านสาระต่อ
            </button>
          </div>
        </div>
...
export function openPostDetail(id) {
  ...
  document.getElementById('detail-image').src =
    safeUrl(post.image) || 'https://placehold.co/800x600/1e293b/ffffff?text=Image';
  ...
  if (post.type === 'review' || !post.type) {
    ...
    document.getElementById('detail-product-price').innerText =
      `${Number(post.productPrice || 0).toLocaleString('th-TH')} ฿`;
